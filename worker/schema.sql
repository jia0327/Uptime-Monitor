...
CREATE TABLE monitors (
  ...
  name TEXT NOT NULL,
  url TEXT NOT NULL,                       -- 检测 URL（HTTP 探测用）
  link_url TEXT,                           -- 跳转链接（状态页展示用，检测 URL 不公开）
  method TEXT DEFAULT 'GET',
  request_headers TEXT,                    -- JSON 格式自定义请求头
  request_body TEXT,                       -- POST 请求体
  interval INTEGER DEFAULT 300,             -- 检测间隔（秒），0=不检测（仅作书签）
  ...
  is_private INTEGER DEFAULT 0,             -- 私密监控 (1=公开页隐藏跳转链接)
...
-- 私密监控
-- ============================================================
-- ALTER TABLE monitors ADD COLUMN is_private INTEGER DEFAULT 0;
-- CREATE INDEX IF NOT EXISTS idx_incidents_active ON incidents(type, status, scheduled_start, scheduled_end);
--
-- ============================================================
-- 跳转链接（检测 URL 与点击跳转分离）
-- ============================================================
-- ALTER TABLE monitors ADD COLUMN link_url TEXT;

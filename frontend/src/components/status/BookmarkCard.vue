  ...
  <component
    ...
    <div class="min-w-0 flex-1">
      <div class="flex items-center gap-1.5 min-w-0">
        <span class="font-semibold text-sm text-slate-900 dark:text-white truncate transition-colors"
          :class="clickable ? 'underline underline-offset-[3px] decoration-slate-400/50 group-hover:decoration-blue-400 group-hover:text-blue-500 dark:group-hover:text-blue-300' : ''">{{ monitor.name }}</span>
      ...
      <p v-if="!visitUrl" class="text-[11px] text-slate-500 dark:text-slate-600 mt-0.5">链接未公开</p>
    </div>
...
const linkProps = computed(() => clickable.value
  ? { href: visitUrl.value, target: '_blank', rel: 'noopener', title: visitUrl.value }
  : {});
</script>

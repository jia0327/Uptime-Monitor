    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-xl glass admin-modal rounded-2xl shadow-2xl flex flex-col overflow-hidden" style="animation:modal-in 0.25s ease-out; max-height: 85vh">
        ...
        <div class="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div class="space-y-3">
              ...
              <div v-if="['POST','PUT','PATCH'].includes(form.method)"><label class="block text-xs font-medium text-slate-400 mb-1">请求体</label><textarea v-model="form.request_body" placeholder='{"key":"value"}' rows="2" class="w-full border border-slate-700 rounded-lg px-3 py-2 text-sm bg-slate-800/80 text-white outline-none font-mono placeholder-slate-600 resize-none text-xs"></textarea></div>
            </div>
          </div>
          <!-- 功能开关 -->
          ...
          <div>
            ...
            <div class="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
              <label v-for="opt in intervalOptions" :key="'interval-' + opt.value"
                ...
                <input type="radio" :value="opt.value" v-model.number="form.interval" class="sr-only"><span class="text-sm font-bold">{{ opt.label }}</span>
              </label>
            </div>
              ...
              <div v-for="item in silenceItems" :key="item.key">
                ...
                <div class="grid grid-cols-5 gap-1.5">
                  <label v-for="opt in silenceOptions" :key="item.key + '-' + opt.value"
                    ...
                    :title="opt.hint">
                    <input type="radio" :value="opt.value" v-model.number="form[item.key]" class="sr-only"><span class="text-sm font-bold">{{ opt.label }}</span>
                  </label>
                </div>
              </div>
            </div>
        ...
        <div class="px-6 py-4 border-t border-white/5 bg-slate-900/30">
          <button @click="submitSave" :disabled="configSaving" class="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-2.5 rounded-xl transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer">
...
watch(
  () => [props.configTarget?.id, props.configForm],
  () => {
    if (!props.configForm) return;
    Object.assign(form, normalizeForm(props.configForm));
  },
  { immediate: true },
);
